...
from Secrets.secret import OpenAIKey

class TableExtractor:

    def __init__(self, pdf_path):
        self.openai_api_key = OpenAIKey()  # Replace with your OpenAI API key
        self.llm = ChatOpenAI(
            model="gpt-4o-mini",
    ...
    def _extract_text_from_tables_(self, image: Any, tables: List[Tuple[int, int, int, int]]) -> List[str]:
      ...
      try:
        ...
        for (x, y, w, h) in tables:
            ...
            text = pytesseract.image_to_string(table_image, lang='vie')
            texts.append(text)
    ...
    def key_value_pair(self) -> str:
      ...
      try:
        ...
        key_value=json.dumps(combined_json, ensure_ascii=False, indent=2)
